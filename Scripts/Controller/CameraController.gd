...
func Orbit(direction: Vector2):
	if direction.x != 0:
		...
		headingAngle += direction.x * headingSpeed * \
	...
	if direction.y != 0:
		...
		orbitAngle += direction.y * orbitSpeed * \
		get_process_delta_time()
		
		orbitAngle = clamp(orbitAngle, deg_to_rad(_minPitch),
		deg_to_rad(_maxPitch))
		
		$Heading/Pitch.rotation.x = orbitAngle
	
func _process(delta):
	if _follow:
		self.position = self.position.lerp(_follow.position,\
		 _followSpeed * delta)
